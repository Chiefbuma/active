...
export default function Report({ patient, corporate }: ReportProps) {
  ...
  // Logic to determine the report date based on available data
  let reportDate: Date;
  if (corporate?.wellness_date) {
    reportDate = new Date(corporate.wellness_date);
  } else if (clinical?.created_at) {
    reportDate = new Date(clinical.created_at);
  } else if (nutrition?.created_at) {
    reportDate = new Date(nutrition.created_at);
  } else if (vitals?.measured_at) {
  ...
  const mainDoctor = "Emily Carter"; // Placeholder
