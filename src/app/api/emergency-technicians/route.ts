...
export async function GET() {
  try {
    const [rows] = await db.query('SELECT * FROM emergency_technicians ORDER BY name ASC');
...
export async function POST(req: Request) {
  try {
    ...
    const [rows] = await db.query('SELECT * FROM emergency_technicians WHERE id = ?', [insertId]);
