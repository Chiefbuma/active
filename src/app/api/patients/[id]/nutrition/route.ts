...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const [result] = await connection.query(
      ...
      [patient_id, height, weight, bmi, visceral_fat, body_fat_percent, notes_nutritionist]
    );
    connection.release();

    const insertId = (result as any).insertId;
    const [rows] = await connection.query('SELECT * FROM nutrition WHERE id = ?', [insertId]);
